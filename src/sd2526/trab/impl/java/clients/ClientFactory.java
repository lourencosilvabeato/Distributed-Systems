...
public class ClientFactory<T> {
	...
	public T get(String domain) {
		...
		return get(uris[ThreadLocalRandom.current().nextInt(uris.length)]);
	}

	public T get(String domain, int preferredIndex) {
		var sn = "%s@%s".formatted(serviceName, domain);
		var raw = Discovery.getInstance().knownUrisOf(sn, 1);
		var uris = Arrays.stream(raw).filter(u -> u != null).sorted().toArray(URI[]::new);
		if (uris.length == 0) return get(domain);
		return get(uris[Math.abs(preferredIndex) % uris.length]);
	}
